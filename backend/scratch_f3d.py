...
for D in range(20, 60, 2):
    for X in range(0, 7*D+1, 10):
        c = mk(X, D, 188.0, 200.0)
        rq = required_quantity(c)
        if 480 <= rq <= 560:
            ranked = rank(simulate_all(c, recommended_qty=800))
            need_r = next((r for r in ranked if r.candidate.action_type==ActionType.CREATE_PO and r.candidate.qty==rq or (r.candidate.action_type==ActionType.CREATE_PO and abs(r.candidate.qty-rq)<60)), None)
            eight = next((r for r in ranked if r.candidate.action_type==ActionType.CREATE_PO and r.candidate.qty in (750,800,850)), None)
            if need_r and need_r.feasible and eight and not eight.feasible and 'capacity' in eight.binding_constraints:
                print(D, X, rq, "need_qty=", need_r.candidate.qty, "need_feasible", need_r.feasible, "eight_qty", eight.candidate.qty, eight.binding_constraints)
