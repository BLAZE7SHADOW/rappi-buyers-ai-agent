...
from app.domain.candidates import PlanningContext, simulate_all, rank, required_quantity, round_to_pack, baseline_projection
...
for D,X in [(30,150),(30,180),(30,200),(25,150),(20,120),(20,100)]:
    c = mk(X, D, 188.0, 200.0)
    rq = required_quantity(c)
    ranked = simulate_all(c, recommended_qty=800)
    print(f"D={D} X={X} rq={rq}")
    for r in ranked:
        if r.candidate.action_type == ActionType.CREATE_PO:
            print("   qty",r.candidate.qty,"feasible",r.feasible,r.binding_constraints,"peak", r.after.peak_occupancy_m3)
